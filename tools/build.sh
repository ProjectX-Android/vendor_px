...
opt_fls=0
  ...
  while getopts "c:h:j:l:n:s:t:u" opt; do
    case "$opt" in
      c) opt_clean="$OPTARG" ;;
      h) usage ;;
      j) opt_jobs="$OPTARG" ;;
...
rm -f "$OUTDIR/target/product/$device/obj/KERNEL_OBJ/.version"
  ...
  if [[ ! -d prebuilts/gcc/linux-x86/arm/arm-linux-androideabi-4.9-sm ]]; then
    opt_tc=1
  elif [[ ! -d prebuilts/gcc/linux-x86/arm/arm-linux-gnueabi-4.9-sm ]]; then
    opt_tc=1
  fi
  ...
  if [[ $opt_tc -eq 1 ]]; then
	...
	shft_tc=$(( shft_tc + 1 ))
	echo ""
	read -p "${bldgrn}Enter toolchain ${shft_tc}'s name (ex. arm-linux-gnueabi-4.9): ${txtrst}" bld_tc
	grep "arm-" $bld_tc &>/dev/null
	FCheck=$?
	...
	echo ""
	echo "${txtblu}Moving $HOME/tmp/$bld_tc to $src_dir/prebuilts/gcc/linux-x86_64/$fldr/${bld_tc}-sm...${txtrst}"
	mv "$HOME/tmp/$bld_tc" "$src_dir/prebuilts/gcc/linux-x86_64/$fldr/${bld_tc}-sm"
	echo ""
