  ...
  if [[ $opt_tc -eq 1 ]]; then
    ...
    shft_tc=0
	  ...
	  if [[ $no_tc1 -eq 1 ]]; then
	    ...
	    bash arm-linux-androideabi-4.9
           ...
           echo "${blu}Moving $HOME/tmp/arm-linux-androidabi-4.9 to $DIR/prebuilts/gcc/linux-x86/arm/arm-linux-androideabi-4.9-sm...${txtrst}"
	   mv "$HOME/tmp/arm-linux-androideabi-4.9" "$DIR/prebuilts/gcc/linux-x86/arm/arm-linux-androideabi-4.9-sm"
	   FCheck=$?
	  ...
	  if [[ $no_tc2 -eq 1 ]]; then
	    ...
	    bash arm-eabi-4.9
           ...
           echo "${blu}Moving $HOME/tmp/arm-gnueabi-4.9 to $DIR/prebuilts/gcc/linux-x86/arm/arm-eabi-4.9-sm...${txtrst}"
           mv "$HOME/tmp/arm-linux-gnueabi-4.9" "$DIR/prebuilts/gcc/linux-x86/arm/arm-eabi-4.9-sm"
           FCheck=$?
	...
	echo ""
	  ...
	  if [[ $bld_tc = arm-eabi-4.9 ]]; then
            echo "${blu}Moving $HOME/tmp/arm-linux-gnueabi-4.9 to $DIR/prebuilts/gcc/linux-x86/arm/arm-eabi-4.9-sm...${txtrst}"
	    mv "$HOME/tmp/arm-linux-gnueabi-4.9" "$thisDIR/prebuilts/gcc/linux-x86/arm/arm-eabi-4.9-sm"
	    FCheck=$?
	  else
	    echo "${blu}Moving $HOME/tmp/$bld_tc to $DIR/prebuilts/gcc/linux-x86/$fldr/${bld_tc}-sm...${txtrst}"
	    mv "$HOME/tmp/$bld_tc" "$DIR/prebuilts/gcc/linux-x86/$fldr/${bld_tc}-sm"
	    FCheck=$?
	  fi
	echo ""
