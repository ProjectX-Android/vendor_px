...
usage()
 {
   ...
   echo "  ./build.sh [options] [device] [variant]"
   echo ""
...
variant="$2"
  ...
  if [[ -z $variant ]]; then
    echo "${bldred}No build variant specified${txtrst}"
    usage
  fi
...
yn_check()
{
 until [[ $yorn -eq y || $yorn -eq n ]]; do
  ...
  if [[ ! -d prebuilts/gcc/linux-x86/arm/arm-eabi-4.9 ]]; then
    ...
    read -p "Do you want to build it?" yorn
    yn_check
      if [[ $yorn -eq n ]]; then
...
echo ""
  ...
  if [[ $opt_log -eq 2 || $opt_log -eq 3 ]]; then
    ...
    lunch "px_$device-$variant" && make "-j$opt_jobs" > "$HOME/make_$device.log"
  else
    echo "${bldblu}Compiling ROM${txtrst}"
    lunch "px_$device-$variant" && make "-j$opt_jobs"
  fi
FCheck=$?
