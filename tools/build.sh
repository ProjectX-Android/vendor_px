...
thisDIR="${PWD##*/}"
  ...
  if [[ -z $OUT_DIR_COMMON_BASE ]]; then
    ...
    echo "${cya}But $OUTDIR/$thisDIR will be used${txtrst}"
    echo ""
  fi
...
no_tc2=0
  ...
  while getopts "c:hj:l:nst:u" opt; do
    case "$opt" in
      c) opt_clean="$OPTARG" ;;
      h) usage ;;
      j) opt_jobs="$OPTARG" ;;
...
# Check if proprietary files are downloaded
  ...
  if [[ $opt_fls -eq 0 && ! -d vendor/$dev_dir/$device ]]; then
    echo "${bldred}No proprietary files found!${txtrst}"
    echo "${bldred}If this is the first time you're building this device use ./build.sh -n $device ${txtrst}"
    exit 7
  fi
...
rm -f "$OUTDIR/target/product/$device/obj/KERNEL_OBJ/.version"
  ...
  if [[ ! -d prebuilts/gcc/linux-x86/arm/arm-linux-androideabi-4.9-sm ]]; then
    ...
    no_tc1=1
    mch_tc=1
    echo "${bldblu}Toolchain arm-linux-androideabi-4.9 not found, will be built!${txtrst}"
  fi
  ...
  if [[ ! -d prebuilts/gcc/linux-x86/arm/arm-eabi-4.9-sm ]]; then
    ...
    echo "${bldblu}Toolchain arm-eabi-4.9 not found, will be built!${txtrst}"
  fi
  ...
  if [[ $opt_tc -eq 1 ]]; then
    ...
    shft_tc=0
	  ...
	  if [[ $no_tc1 -eq 1 ]]; then
	    echo "${bldblu}Building arm-eabi-4.9 Toolchain because it is missing in the source code...${txtrst}"
           ...
           echo "${blu}Moving $HOME/tmp/arm-gnueabi-4.9 to $thisDIR/prebuilts/gcc/linux-x86/arm/arm-eabi-4.9-sm...${txtrst}"
	   mv "$HOME/tmp/arm-gnueabi-4.9" "$thisDIR/prebuilts/gcc/linux-x86/arm/arm-eabi-4.9-sm"
	   FCheck=$?
	  ...
	  if [[ $no_tc2 -eq 1 ]]; then
            ...
            bash arm/arm-linux-androideabi-4.9
            FCheck=$?
           ...
           echo "${blu}Moving $HOME/tmp/arm-linux-androideabi-4.9 to $thisDIR/prebuilts/gcc/linux-x86/arm/arm-linux-androideabi-4.9-sm...${txtrst}"
           mv "$HOME/tmp/arm-linux-androideabi-4.9" "$thisDIR/prebuilts/gcc/linux-x86/arm/arm-linux-androideabi-4.9-sm"
           FCheck=$?
	...
	echo ""
	  ...
	  if [[ $bld_tc = arm-eabi-4.9 ]]; then
            echo "${blu}Moving $HOME/tmp/arm-linux-gnueabi-4.9 to $thisDIR/prebuilts/gcc/linux-x86/arm/arm-eabi-4.9-sm...${txtrst}"
	    mv "$HOME/tmp/arm-linux-gnueabi-4.9" "$thisDIR/prebuilts/gcc/linux-x86/arm/arm-eabi-4.9-sm"
	    FCheck=$?
	  else
	    echo "${blu}Moving $HOME/tmp/$bld_tc to $thisDIR/prebuilts/gcc/linux-x86/$fldr/${bld_tc}-sm...${txtrst}"
	    mv "$HOME/tmp/$bld_tc" "$thisDIR/prebuilts/gcc/linux-x86/$fldr/${bld_tc}-sm"
	    FCheck=$?
	  fi
	echo ""
...
echo ""
  ...
  if [[ $opt_log -eq 2 || $opt_log -eq 3 ]]; then
    ...
    lunch "px_$device-userdebug" && make $device "-j$opt_jobs" > "$HOME/make_$device.log"
  else
